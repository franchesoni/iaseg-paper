...
def annotate_img(img, preannotation, target, max_delay, threshold=0.9):
  ...
  if np.sum(target) == 0:
    if np.abs(np.sum(pred_mask)) > 0.001:
      time.sleep(max_delay * np.random.rand())  # delay because we need to clear the bad prediction
    pred_mask = np.zeros_like(np.array(target))
  else:
    while IoU(pred_mask, target) < threshold and number_of_clicks < 20 and np.sum(target) != 0:
...
def annotate_data(input_dir, target_dir, annotations_dir, ckpt_path=Path('checkpoints'), label=10, max_delay=4):
  ...
  for i, (img, target) in enumerate(dataset):
    ...
    if (Path(annotations_dir) / f"annotation_{str(i).zfill(6)}_{str(label).zfill(2)}.png").exists():
    ...
    print("Annotated image", i, "with label", label, "and checkpoint", ckpt_path, "at time", datetime.datetime.utcnow())
