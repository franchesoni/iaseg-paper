...
class ContinuousDataset(torch.utils.data.Dataset):
    ...
    def update_file_list(self):
        ...
        annotation_indices = [
            ann.name.split("_")[1]
            for ann in self.annotations_dir.glob(
                f"annotation_*_{str(self.label).zfill(2)}.png"
            )
        ]
        self.common_indices = sorted(
            list(set(img_indices).intersection(set(annotation_indices)))
        )
        if len(self.common_indices):  # shouldn't be 0
            self.common_indices = self.common_indices * (
                self.min_length // len(self.common_indices) + 1
            )
    ...
    def __getitem__(self, idx):
        image = np.array(
            Image.open(self.input_dir / f"img_{self.common_indices[idx]}.png")
        )
        mask = (
            np.array(
                Image.open(
                    self.annotations_dir
                    / f"annotation_{self.common_indices[idx]}_{str(self.label).zfill(2)}.png"
                )
            )
            != 0
        ).astype(np.uint8)
        ...
        with tqdm.tqdm(enumerate(dataloader)) as pbar:
            for i, (img, target) in pbar:
                ...
                loss = torchvision.ops.sigmoid_focal_loss(
                    out, target[:, None, :, :].float(), reduction="mean"
                )
